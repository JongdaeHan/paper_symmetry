...
% cifar100의 라벨에 비해 데이터의 수가 적기 때문에 주어진 에폭 내에 학습이 잘 이루어지지 않음(추가필요). 그렇기 때문에 30에폭일 때 모델이 사용되었음(아래)
The reason for the low result in CIFAR-100 is that the model with more than 90\% accuracy within the given 30 epochs was not used. % The reason for the low results is that unlike cifar10, which used a model with an accuracy of over 90% within a given 30 epoxy, cifar100 did not have a model with an accuracy of over 90% within a given epoxy, so it used a model with a different dataset and 30 epoxy.
% 그럼에도 불구하고 적대적 학습을 진행하면 cifar10과 마찬가지로 적대적 예시가 더 많이 포함될 수록 정확성이 소폭 감소하고 안전성이 대폭 향상되는 경향을 확인할 수 있음
