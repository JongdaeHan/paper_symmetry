...
In Figure~\ref{cifar100-0.1-graph}, the results for the CIFAR-100 {\it adv\_eval} made with \begin{math}\epsilon\end{math} 00.1 can be visually confirmed.
...
% 그 외에 모델들에 대해서는 이전 데이터셋과 마찬가지로 20 모델과 큰 성능 차이는 보이지 않지만 유사한 경향성이 발견되는 것을 확인할 수 있다.(여기부터 작업 진행)
