...
\address{%
$^{1}$ \quad Affiliation 1; aggsae@gmail.com\\
...
\caption{Examples of adversarial examples.\\(a) CIFAR-10 (b) CIFAR-100 (c)GTSRB\label{adversarial-examples}}
\end{figure} 
...
The generated adversarial training datas are randomly extracted at a certain ratio and mixed with the original training datas to be used for training. And the adversarial evaluaion datas are used for experiments to measure the safety.

\subsection{Methodology} % START :: 여기부터 작업 시작
...
In this step, we trained the model only with the original train datas for being baseline of accuracy and safety performance, first. We then generate the mixed training dataset that mixes original train datas with adversarial train datas.
...
The {\it N model} means a model trained on mixed training datas containing N\% of total number of traninig datas number of adversarial examples.
...
Although the same equation is applied, we distinguish between the two measurements depending on whether the evaluation datas are in the {\it orig\_eval} or {\it adv\_eval}.
...
In the experiments, it can be seen that it is essential to mix the adversarial examples from training datas at least 20\% when construct the training dataset in order to verify the safety.
